...
sub _GetStatStr($$;$$)
{
      ...
      exists $Stats->{"$StatKey.count"})
  {
    ...
    return $Avg eq "n/a" ? $Avg : _GetDuration($Avg, $Flags & $NO_TIME);
  }
...
sub GenerateBody($)
{
  ...
  _GenGlobalLine($GlobalStats, "jobs.time", "Job completion", "How long it takes to complete a  regular job (excluding canceled ones).");
  ...
  print "<tr><td class='StatSeparator'>Maximum times</td><td colspan='2'><hr></td></tr>\n";
  _GenGlobalLine($GlobalStats, "jobs.time.max", "Job completion", "How long the slowest job took.");

  print "<tr><td class='StatSeparator'>VMs</td><td colspan='2'><hr></td></tr>\n";
  my $VMsStats = $Stats->{vms};
  _GenGlobalLine($VMsStats, "elapsed", "VM history", "How far the VM and VM host history record goes. This is used for the tables below.");
