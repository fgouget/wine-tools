...
sub _CheckAndClassifyVMs($)
{
  ...
  foreach my $VM (@{$VMs->GetItems()})
  {
    ...
    if (!$VM->HasEnabledRole())
    {
      # Don't schedule anything on it
      $Sched->{busyvms}->{$VMKey} = 1;
    }
    elsif ($VM->HasRunningChild())
    {
...
sub _ScheduleTasks($$)
{
  ...
  # A VM was in an inconsistent state so check if that impacts a Task and
  # update its Status (all the way up to the Job) before scheduling Tasks
  map { $_->UpdateStatus(); } @{$Jobs->GetItems()} if ($Sched->{vmerror});
    ...
    foreach my $Step (@SortedSteps)
    {
      ...
      foreach my $Task (@SortedTasks)
      {
        ...
        if (!_AddNeededVM($NeededVMs, $VM, $JobRank))
        {
          ...
          # So it is nowhere near ready to run this Task and thus there is
          # no point preparing the next step.
          $StepVMs = undef;
          next;
        }
  ...
  foreach my $VM (@{$Sched->{VMs}->GetItems()})
  {
    ...
    my $VMKey = $VM->GetKey();
    my $Niceness = $FUTURE_BASE + 10 * $Sched->{nicefuture}->{$VMKey} +
                   ($VM->Status eq "idle" ? 0 : $VM->Status ne "dirty" ? 1 : 2);
...
sub _SacrificeVM($$$)
{
  ...
  foreach my $CandidateKey (keys %{$Sched->{lambvms}})
  {
    ...
    next if (_CompareNeededVMs($NeededVMs, $CandidateKey, $VM->GetKey()) <= 0);
...
sub _RevertVMs($$)
{
  ...
  foreach my $VMKey (@SortedNeededVMs)
  {
    ...
    if (_GetNiceness($NeededVMs, $VMKey) >= $FUTURE_BASE)
    {
      ...
      if ($FutureActive > $Host->{MaxVMsWhenIdle})
      {
        # The host has already reached its limit for future VMs.
        next;
      }
    }
    ...
    if ($FutureActive <= $Host->{MaxActiveVMs})
    {
      delete $Sched->{lambvms}->{$VMKey};
      $Sched->{busyvms}->{$VMKey} = 1;
...
sub ScheduleJobs()
{
  ...
  # the scheduler.
  map { $_->RecordStatus($Sched->{records}) } @{$VMs->GetItems()};
  $Sched->{recordgroups}->Save();
